...
class PointNetVAE(pl.LightningModule):
    def __init__(self, latent_dim, optimizer, optimizer_param, seq_len = 500, amino_acids = 21, hidden_dim=512, beta = 0, beta_increment = 1, beta_epoch_start = 20, beta_cycle = 20, conv_hidden_dim = 128, global_feature_size = 512, reconstruction_loss_weight = 1):
        ...
        self.conv3 = torch.nn.Conv1d(conv_hidden_dim*2, conv_hidden_dim*4, 1)
        ...
        self.fc_point_dec = nn.Linear(self.hidden_dim, 3*seq_len)

        self.seq_dec_1 = nn.Linear(self.latent_dim, self.hidden_dim)
        self.sequence_decoder = nn.Linear(self.hidden_dim, self.amino_acids*seq_len)
    ...
    def encode(self, x, labels):
        ...
        x = self.tanh(self.conv1(x.permute(0,2,1)))
    ...
    def decode(self, z):
        ...
        # Point Decoding Branch
        point = self.tanh(self.point_dec_1(z))
        point = self.fc_point_dec(point)
        point = point.reshape(-1,self.seq_len, 3)
        
        # Sequence Decoding Branch
        logit = self.tanh(self.seq_dec_1(z))
        logit = self.sequence_decoder(logit)
        logit = logit.reshape(-1,self.seq_len,self.amino_acids)
        
        softmax_logits = self.soft(logit)
        z = torch.cat((point, softmax_logits), dim=-1)  

        logit = torch.cat((point, logit), dim=-1)
        return z, logit
    ...
    def ELBO(self, x, logit, x_mu, x_logvar):
        ...
        chamfer = 3*self.calculate_mse(predicted_coords, x[:,:,:3], mask) #self.calculate_chamfer_distance(predicted_coords, x[:,:,:3], mask)

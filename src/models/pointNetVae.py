...
class PointNetVAE(pl.LightningModule):
    def __init__(self, latent_dim, optimizer, optimizer_param, seq_len = 500, amino_acids = 21, hidden_dim=512, dropout = 0.4, beta = 1, embed_dim = 64, global_feature_size = 512):
        ...
        self.embedding = nn.Embedding(21,embed_dim,20)
        ...
        self.fc1_enc_mu = nn.Linear(self.global_feature_size, self.latent_dim)
        self.fc1_enc_logvar = nn.Linear(self.global_feature_size, self.latent_dim)
        ...
        self.fc4_dec = nn.Linear(self.latent_dim*8, self.seq_len*3)
    ...
    def forward(self, x):
        ...
        reparam_z, x_mu, x_logvar, input_transform, feature_transform = self.encode(x, labels)
        x_rec = self.decode(reparam_z)

        return reparam_z, x_mu, x_logvar, x_rec, input_transform, feature_transform
    ...
    def encode(self, x, labels):
        ...
        x = self.bn2(self.relu(self.shared_conv1d_2(x+labels)))
        ...
        return reparam_z.squeeze(), x_mu.squeeze(), x_logvar.squeeze(), t_net_transform, t_net_feature_transform
    ...
    def ELBO(self, x, x_reconstructed, x_mu, x_logvar):
        ...
        rec_loss =  torch.sum(self.chamfer_distance(x, x_reconstructed))
    ...
    def training_step(self, batch, batch_idx):
        ...
        x = batch
        rep_z, x_mu, x_logvar, x_rec, input_transform, feature_transform = self(x)
        loss, rec_loss, KL_loss = self.ELBO(x[:,:,:3], x_rec, x_mu, x_logvar)
